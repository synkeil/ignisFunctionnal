...
const dom = function returnDomElement(main) {
  const toSelect = main[0];
  return toSelect;
};

const eq = function atIndex(...theArgs) {
...
const attr = function setAttribut(atr, val) {
  selected.map(
    (x) => {
      let temp = x;
...
const get = function returnSelectedArray() {
  return selected;
};

// set the css of the iQ in the form of "color:blue;width:125px"
const css = function setStyle(style) {
  selected.map(
    (x) => {
      let temp = x;
...
const addClass = function addClassToElement(toAdd) {
  selected.map(
    (x) => {
      let temp = x;
...
const removeClass = function removeClassFromElement(toRemove) {
  selected.map(
    (x) => {
      let temp = x;
...
const html = function setContentAbsolutly(...theArgs) {
...
const val = function returnElemValue(...theArgs) {
  a = theArgs.length > 0 && theArgs[0] !== undefined ? theArgs[0] : null;
...
const listen = function addEvent(evnt, func) {
  selected.map(
    (x) => {
      let temp = x;
...
const append = function addContentToExisting(code) {
  selected.map(
    (x) => {
      let temp = x;
      temp.innerHTML += code;
...
const ajax = function getAndPost(...theArgs) {
